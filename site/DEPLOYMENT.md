...
- Google Cloud project: `ibq-websites`. The owner approved Google's User Data Policy and created `IBQ Websites — Web`. Its ID and secret are configured locally and in Vercel's production environment. End-to-end owner Google login on the custom domain is verified: the dashboard greets Nick, the admin page lists the one cloud user, and a full page reload retains authenticated admin access.
...
Vercel issued a managed certificate for both names. Real HTTPS requests to the apex home, www home, login, and Auth.js provider endpoint succeeded with HTTP 200. The www domain now has a Vercel 308 redirect to the apex, verified on `/login`, so authentication cookies consistently use `ibqwebsites.com` and do not split between hosts. The owner completed Google sign-in on the live domain using the correct HTTPS callback and only openid/profile/email scopes. Both `/account` and `/admin` load authenticated; a full reload of `/admin` retained the session. The live admin shows one user and zero tickets, confirming cloud user creation and database-backed admin reads without demo seeding. Google app publication/testing restrictions for other customers still need review before public onboarding.
...
Local fake payments require `ALLOW_LOCAL_TEST_PAYMENTS=true`. They are always disabled in production. Missing Stripe credentials therefore cannot silently unlock paid deliverables. Custom-domain DNS/TLS and owner Google login/admin access are verified. Stripe keys/webhooks, production notification email, Google onboarding for non-owner customers, and a full customer purchase/delivery flow still need setup or live verification before accepting customers/payments.
